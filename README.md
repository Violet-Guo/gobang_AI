...
搜索到D的时候，局面得分是5，（顺便说一句，这样的搜索是深度优先搜索，什么是深度优先搜索，可百度）那么也就是说对于节点C是要
搜索最大值，那么只可能会在（5，+∞）之间， 同理，对于节点B来说是要搜索最小值，也只会在（-∞，5）之间。 
继续搜索，当搜索到G时，F暂时等于6 ，对于节点F来说是要找最大值， 那么F不可能再小于6了， 而对于节点B是要找最小值的，
B的已知最小值是在（-∞，5）之间的， 而节点F的值是6并且F不可能比6再小了，相当于是不符合节点B要找的最小值，那就相当于在F节点
之后继续搜索是没有意义的，于是果断咔嚓掉F这个分支，不搜索了，这就是剪枝。 
同样对于另外一边的已知可能的极限范围β也是一样的情况，遇到就算是搜索也是浪费时间的情况，就剪枝不搜索了。 
这样就减少了很多不必要是搜索步骤，特别是一开始就找到最有可能的极大极小值，更能迅速的剪枝。怎么一开始尽快的找到可能的极大
极小值呢，后面再说。先插播一下，负值极大法。
